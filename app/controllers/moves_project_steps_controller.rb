...
class MovesProjectStepsController < ApplicationController
  ...
  def set_frame_updated
    ...
    @servers = @moves_project_step.servers_at_current_step_for(@frame)
    @moves = @moves_project_step.moves_for_frame_at_current_step(@frame)
    @move_connections = Move::Connection.where(move: @moves)
  end
end
