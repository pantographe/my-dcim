...
RSpec.describe Server do
  ...
  describe ".friendly_find_by_lower_or_name" do
    context "with id" do
      it { expect(described_class.friendly_find_by_lower_or_name(1)).to eq(servers(:one)) }
    end

    context "with numero" do
      it { expect(described_class.friendly_find_by_lower_or_name("ABCDEFG32")).to eq(servers(:two)) }
    end

    context "with slug" do
      it { expect(described_class.friendly_find_by_lower_or_name("server-name-4")).to eq(servers(:four)) }
    end
  end
